...
function startQuiz() {
    ...
    for (var i = 0; i < questions[indexNumber].options.length; i++) {
        answerParent.innerHTML += `<div class="col-md-6 py-2"><button onclick="checkQuestion(this, '${questions[indexNumber].options[i]}')" class="btn w-100 btn-info">${questions[indexNumber].options[i]}</button></div>`;
    }
